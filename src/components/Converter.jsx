...
const example = {value: JSON.stringify({"foo": 1, "bar": [2, 3], "baz": {"qux": true}}, null, 2), type: "json"};
        ...
        return (
            <React.Fragment>
                <Navbar bg="light" expand="lg">
                    <Navbar.Brand href="http://bit.ly/cnvcode"><i style={{color: "red"}}
                                                                  className="fa fa-code"/> http://bit.ly/cnvcode</Navbar.Brand>
                    <Navbar.Toggle/>
                    <NavbarCollapse className="justify-content-end">
                        <Nav.Item>
                            <Nav.Link href='https://github.com/alexec/cnv' title='Github'><i className="fa fa-github"/></Nav.Link>
                        </Nav.Item>
                        <Nav.Item>
                ...
                {this.state.stack.map((entry, i) => (
                        <Container key={`container-${i}`} fluid={true}>
                                ...
                                <Col>
                                    <ButtonToolbar
                                        ...
                                        <ButtonGroup>
                                            ...
                                            {["text", "json", "yaml", "base64", "jwt"].map(type => (
                                                <Button key={`button-convert-${i}-${type}`} variant="secondary"
                                                        onClick={() => {
                                                            this.convert(type);
                                                        }}>{type}</Button>
                                            ))}
                                        ...
                                        <ButtonGroup>
                                            ...
                                            <CopyToClipboard text={this.state.stack[i].value} onCopy={() => {
                                                ...
                                                <Button variant="secondary" disabled={this.state.copied}><i
                            ...
                            <Row>
                                <Col>
                                    <Form.Control as='textarea' rows={10} key={`editor-${i}`}
